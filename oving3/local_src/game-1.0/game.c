...
void moveball(){
    ...
    if(ball_a.Ypos + ball_a.radius < 20)ball_a.Yspeed = -ball_a.Yspeed; //Bounces the ball from the top
    if(ball_a.Ypos - ball_a.radius > playfield_a.height - 20)ball_a.Yspeed = -ball_a.Yspeed; // Bounces the ball from the bottom
}
...
void handlePhysics(){
    ...
    if (ball_a.Xpos + ball_a.radius >= 290) {   //Checks if the ball hits the bat
